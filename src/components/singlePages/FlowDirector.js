  ...
  componentWillMount() {
    ...
    generalUtils.storageGetItem('todayFlow').then((todayFlow) => {
      ...
      for(var i=0; i<todayFlow.length; i++){
        if(this.contin){
        if((todayFlow[i][1] === 0 &&  this.faks > todayFlow[i][0]) || (todayFlow[i][1] === 0  && this.checkday !== this.newDate )){ // 0 means not finished and > means time passed
          if(todayFlow[i][2]===0){
      ...
      if (this.missed === 0) {
        ...
        if (this.checkday !== this.newDate) {
          Object.keys(this.showVSquiz).forEach(function(key, index) {
